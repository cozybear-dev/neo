...
export function sandboxContainer(env = process.env) {
    const value = env.SANDBOX_CONTAINER?.trim();
    return value || DEFAULT_SANDBOX_CONTAINER;
}
export function dockerSocketPath(env = process.env) {
...
export function dockerExecArgs(container, spec) {
...
export function decodeDockerMux(buf) {
    const view = buf;
...
export async function spawnCollect(command, args, opts = {}) {
...
async function httpExec(container, spec, opts) {
    ...
    let created;
    try {
        created = JSON.parse(create.body.toString('utf8'));
    }
    catch {
    ...
    if (!execId)
        throw new Error('docker exec create did not return Id');
    ...
    if (inspect.status >= 200 && inspect.status < 300) {
        try {
            const info = JSON.parse(inspect.body.toString('utf8'));
...
async function cliExec(container, spec, opts) {
...
export function createDocker(opts = {}) {
    return {
        async exec(container, spec, signal) {
            const merged = { ...opts, signal: signal ?? opts.signal };
            if (opts.engine)
                return httpExec(container, spec, merged);
            if (opts.spawn)
...
export async function execInSandbox(args, opts = {}) {
    throwIfAborted(opts.signal);
